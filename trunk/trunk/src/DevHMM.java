...
public class DevHMM extends DevHMMPre {

    public DevHMM(Trainer tr) {
    ...
    protected void paramTest(String w, Classificacio[] m) {
        if (this.frequency || !(calculat.containsKey(w))) {
            ...
            //if (dict.getWord(w) > 5) {
                ...
                double resultat = hmmEnd.wordProb(w) * (1 + w.length()) * lambda;
    ...
    public double logViterbiLikelihood(String fileName) {
        ...
        try {
            while ((word = txreader.nextWord()) != null) {
                ...
                double d = hmmEnd.probViterbi(word);
                value -= Math.log(d);
                System.out.println(hmmEnd.split(word)+" ("+d+")");
            }
    ...
    public double probViterbi(String word) {
        return hmmEnd.probViterbi(word);
    }
    
     */

    public String split(String word){
        return hmmEnd.split(word);
    }
}
